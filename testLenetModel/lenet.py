...
class Lenet:
    def __init__(self):
        # self.raw_input_image = tf.placeholder(tf.float32, [None, 784],name="data")
        # self.input_images = tf.reshape(self.raw_input_image, [1, 28, 28, 1])
        self.input_images = tf.placeholder(tf.float32, [cfg.BATCH_SIZE, 28, 28, 1],name="data")
        self.raw_input_label = tf.placeholder("float", [None, 10])
        self.input_labels = tf.cast(self.raw_input_label,tf.int32,name="output")
    ...
    def construct_net(self,is_trained = True):
        with slim.arg_scope([slim.conv2d], padding='VALID',
            ...
            net = slim.conv2d(self.input_images,20,[5,5],1,padding='SAME',scope='conv1')
